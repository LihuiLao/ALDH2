...
group_list <- list(
  ...
  "Control" = ctl_cols,
  "NewGroup" = ng_cols,
  "Group5" = group5_cols  # <-- 已添加第五组
)

# 合并所有强度相关的列名 (此部分无需修改，会自动更新)
intensity_cols <- unlist(group_list)

# 定义组名和颜色 (此部分无需修改，会自动更新)
...
# --- 用户操作提示 ---
# 请确保您的 'analysis.xlsx' 文件中的 'lfq_analysis' sheet 包含了新增第五组的所有样本数据
...
if (!all(intensity_cols %in% colnames(df))) {
  stop("错误: Excel文件中缺少部分样本列，请检查第2部分的 'group_list' 定义和您的Excel文件内容是否一致。")
}
...
# --- 用户操作提示 ---
# 如果您需要进行此项分析，请确保 'fatty_acid_related' sheet 中也包含了第五组的样本数据
